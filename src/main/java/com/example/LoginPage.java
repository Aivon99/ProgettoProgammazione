...
public class LoginPage {
    ...
    private boolean AccountEsiste(String email, String password) {
    ...
    private boolean handleLogin(String email, String password, Stage primaryStage) {
        List<Account> accounts = accountManager.loadAccounts();
        for (Account account : accounts) {
            if (account.getEmail().equals(email) && account.verifyPassword(password)) {
    ...
    private void handleCreateAccount(String nome, String email, String password, Stage createAccountStage) {
        ...
        if(AccountEsiste(email, password)){
            showAlert("Errore", "Errore nella creazione dell'account. Credenziali già utilizzate"); 
        }
        else{if (nome.isEmpty() || email.isEmpty() || password.isEmpty()) {
